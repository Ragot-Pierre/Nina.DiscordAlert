...
namespace NINA.DiscordAlert.DiscordWebhook {
    public class DiscordHelper : IDiscordHelper {
        public async Task SendMessage(MessageType type, string message, ISequenceEntity sequenceItem, CancellationToken cancelToken, ImagePatterns templateValues = null, string attachedFilename = null, Exception exception = null, bool plainText = true)
        {
            ...
            using (var client = Factories.DiscordClient.Create())
            {
                ...
                if (plainText)
                {
                    ...
                    Logger.Debug($"Sent plain text message: {message}");
                    return;
                }
